#_________ЛИНЕЙНАЯ РЕГРЕССИЯ_________
...
beta = least_squares_fit(rescaled_x, y, learning_rate, 1000, 1)
...
def negative_log_lokelihood(xs: List[Vector],
                            ys: List[float],
                            beta: Vector) -> float:
...
def _negative_log_partial_j(x: Vector, y: float, beta: Vector, j: int) -> float:
    """j-ая частная производная для одной точки данных.
...
# подбор случайной начальной точки
...
with tqdm.trange(5000) as t:
    for epoch in t:
        ...
        loss = negative_log_lokelihood(x_train, y_train, beta)
...
for x_i, y_i in zip(x_test, y_test):
    ...
    if y_i == 1 and prediction >= 0.5:    # TP: оплачен, и предсказано,
        true_positives += 1               # что оплачен
    elif y_i == 1:                        # FN: оплачен, и предсказано,
        false_negatives += 1              # что не оплачен
    elif prediction >= 0.5:              # FP: не оплачен, и предсказано,
        false_positives += 1              # что не оплачен
    else:                                 # TN: не оплачен, и предсказано,
